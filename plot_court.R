...
line_points = data.frame(
  x = c(-27,0,27), y = 17.5, desc = "dottedLine"
)
...
court = ggplot() + geom_path(data = court_points,
            ...
            color = "#999999") + scale_linetype_manual(values = c("solid", "longdash"), guide = FALSE) + coord_fixed(ylim = c(0, 30), xlim = c(-27, 27)) +
  theme_court(base_size = 22) + geom_path(data = line_points,aes(x = x, y = y, group = "dottedLine", linetype = "solid"))
